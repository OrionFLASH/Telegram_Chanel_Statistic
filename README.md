...
4. **Настройка конфигурации**:
   ...
   # Список ID каналов/групп для авто-отписки (через запятую)
   # Используется в режиме unsubscribe_only или при полном сканировании
   TELEGRAM_UNSUBSCRIBE_IDS=1523806039,1773402059,2068343941,1078695401,2237736753
   ...
   # Режим работы программы: full (полная обработка), stats_only (только статистика), photos_only (только фотографии), stories_only (только истории), unsubscribe_only (только очистка)
   # По умолчанию: full
   TELEGRAM_WORK_MODE=unsubscribe_only
   ```
   ...
   - `TELEGRAM_WORK_MODE` — режим работы программы (по умолчанию: `full`)
     - `full` — полная обработка: сканирование каналов и чатов, сохранение в Excel, скачивание фотографий профиля и историй
     - `stats_only` — только статистика: сканирование каналов и чатов, сохранение в Excel, без скачивания медиа (фото и истории)
     - `photos_only` — только фотографии: минимальное сканирование личных чатов для получения списка пользователей, скачивание только фотографий профиля (без историй и без сохранения статистики в Excel)
     - `stories_only` — только истории: минимальное сканирование личных чатов для получения списка пользователей, скачивание только историй (без фотографий и без сохранения статистики в Excel)
     - `unsubscribe_only` — только очистка: отписка от каналов из списка `TELEGRAM_UNSUBSCRIBE_IDS`, без сканирования и сохранения данных
